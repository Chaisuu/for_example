def clear_names(file_names: str) -> list:
    ...
    new_name_list = list()
    with open(
        "data/" + file_names, encoding="utf-8"
    ) as names_file:  # Открываем файл и для него создали переменную name_file
        names_list = (
            names_file.read().split()
        )  # мы только что открытый файл читаем и разлеяем на строки
        for (
            name_item
        ) in (
            names_list
        ):  # начинается блок где мы имена т.е. строки чистим, каждую строку итерируем по буквам в строке
            new_name = ""
